...
def apply_change_to_para(para, original_text, replacement_text, rev_id):
    ...
    if pPr is not None:
        children = list(para)
        insert_pos = children.index(pPr) + 1 if pPr in children else 0
    else:
        insert_pos = 0
    ...
    if replacement_text:
        ...
        new_elems.append(ins_elem)
        consumed = 2

    for i, elem in enumerate(new_elems):
        para.insert(insert_pos + i, elem)
...
def apply_all_changes(xml_bytes, changes):
    ...
    for change in changes:
        ...
        for para in tree.iter(w('p')):
            if original in get_para_text(para):
